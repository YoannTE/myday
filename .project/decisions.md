...
- **UI** : titres de sections du Cockpit (MÉTÉO, PLANNING, TÂCHES, NOTES) alignés sur la date du jour de la barre du haut, même police et même taille (`font-display font-bold uppercase`, taille de base, plus de `font-mono text-xs`) ; doublons « ← Cockpit » et `<h1>` Planning/Notes supprimés des en-têtes embarquées ; lien « ← Retour au Cockpit » en haut de /aide ; polices des zones de recherche en `text-xs md:text-sm`.
