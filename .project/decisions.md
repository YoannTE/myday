...
- **Actions dans le bandeau de section** : chaque section (Planning, Tâches, Notes) pose ses propres boutons à droite de son titre via `CockpitSectionActions`, un portail React vers une zone exposée par `CockpitSection`. Choix retenu pour garder l'état (dialogs, rafraîchissements) local à chaque client plutôt que de le remonter dans `cockpit-client`. Les libellés texte « Gérer les catégories » deviennent une roue crantée (`SectionSettingsButton`) et « + Événement » / « + Note » deviennent le bouton rond bleu `SectionAddButton` (porté à `h-8 w-8`).
