...
interface EventCategoryRowProps {
  categorie: EventCategory;
  onChanged: () => void;
}

/**
 * Ligne d'une catégorie dans le dialog de gestion - miroir de
 * `NoteCategoryRow` (notes, Round 015) appliqué aux événements : renommer
 * (clic sur le nom) et supprimer (avec confirmation inline). La suppression
 * ne touche jamais les événements associés (`categorie_id` repasse à NULL
 * côté backend, `ON DELETE SET NULL`).
 */
export function EventCategoryRow({ categorie, onChanged }: EventCategoryRowProps) {
  ...
  async function renommer() {
    ...
    try {
      await apiCall(`/api/event-categories/${categorie.id}`, {
        method: "PATCH",
        body: { nom: nomNettoye },
      });
  ...
  async function supprimer() {
    ...
    try {
      ...
      toast.success(
        "Catégorie supprimée. Les événements associés sont conservés.",
      );
      onChanged();
    } catch (erreur) {
      toast.error(
